...
var TicTacToe = function(readline) {
	...
	function _getInput(callback) {
		_rl.question('Player ' +  _players[0].p + '\'s move (enter row column): ', function(input) {
	...
	function _checkBoard(player) {
		...
		if (winner) {
			console.log(winner.p + ' wins!');
			console.log(winner);
			main.result(game, winner.type);
			// process.exit();
			return false
		}
	...
	return {
		 ...
		 * are 'human' or 'ai'.
		 */
		start: function(_game, playerTypes) {
			game = _game
			_buildBoard();
...
module.exports.gameStart = function (_game, _p1, _p2){
    ticTacToe.start(_game, [_p1, _p2]);
}
